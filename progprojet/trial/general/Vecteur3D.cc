...
void
Vecteur3D :: set_coord(double newX, double newY, double newZ)
{
...
double
Vecteur3D:: get_x() const
{
    return x;
}

double
Vecteur3D:: get_y() const
{
    return y;
}

double
Vecteur3D:: get_z() const
{
    return z;
}
...
ostream&
Vecteur3D:: affiche(ostream &sortie) const
{
    sortie << "(" << x << ", " << y << ", " << z << ") ";
    return sortie;
}
...
double
Vecteur3D :: norme2() const
{
    return x * x + y * y + z * z;
}

double
Vecteur3D :: norme() const
{
    return sqrt(norme2());
}

void
Vecteur3D :: random_vector() // pour test
{
...
{
    if ( (*this).normalise() !=
      Vecteur3D(1, 0, 0) ) return Vecteur3D(1, 0, 0) - (Vecteur3D(1, 0, 0) * (*this)) * (*this) / norme2();
    else return Vecteur3D(0, 1, 0);
}

Vecteur3D
Vecteur3D :: GS1(Vecteur3D excluded) const
{
...
double
Vecteur3D :: distance(Vecteur3D const& v2) const
{
    return ((*this) - v2).norme();
}

Vecteur3D
Vecteur3D :: rotate(double angle, Vecteur3D axe) const
{
    double x1 = x * (cos(angle) + axe.x * axe.x * (1 - cos(angle))) + y
      * (axe.x * axe.y * (1 - cos(angle)) - axe.z * sin(angle)) + z
      * (axe.x * axe.z * (1 - cos(angle)) + axe.y * sin(angle));
    double y1 = y * (cos(angle) + axe.y * axe.y * (1 - cos(angle))) + x
      * (axe.x * axe.y * (1 - cos(angle)) + axe.z * sin(angle)) + z
      * (axe.y * axe.z * (1 - cos(angle)) - axe.x * sin(angle));
    double z1 = z * (cos(angle) + axe.z * axe.z * (1 - cos(angle))) + x
      * (axe.x * axe.z * (1 - cos(angle)) - axe.y * sin(angle)) + y
      * (axe.y * axe.z * (1 - cos(angle)) + axe.x * sin(angle));
...
bool
Vecteur3D :: operator == (double c) const
{
    return ((*this).norme() == c);
}
...
double
Vecteur3D :: operator * (Vecteur3D const& vecteur) const // prod scalaire
{
    return x * vecteur.x + y * vecteur.y + z * vecteur.z;
}

Vecteur3D
Vecteur3D :: operator ^ (Vecteur3D const& vecteur) const // prod vectoriel
{
...
ostream&
operator << (ostream &sortie, Vecteur3D const& vecteur)
{
...
const Vecteur3D
operator + (Vecteur3D vecteur, Vecteur3D const& autre)
{
    return vecteur += autre;
}

const Vecteur3D
operator * (double scalaire, Vecteur3D vecteur)
{
    return vecteur *= scalaire;
}

const Vecteur3D
operator * (Vecteur3D vecteur, double scalaire)
{
    return vecteur *= scalaire;
}
