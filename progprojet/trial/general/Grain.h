...
public:
    ...
    Grain(Position const& position, SupportADessin* support, Vitesse const& vitesse, double masse_volumique, double rayon,
          Vecteur3D const& force = Vecteur3D(), Medium medium = water);
    virtual ~Grain() {}
    ...
    Medium* get_medium() const; // retourne le medium dans le quelle le grain est
    ...
    bool bouger(double temps); //update the coordinates and velocity of the grain after a period of time

    //method to display the grain's data
    std :: ostream& display(std :: ostream& sortie ) const;

    // method to calculate the mass of the grain
    double calc_mass()const
    {
        return (get_volume()*masse_volumique);
    }

	// methodes pour ajouter les force qui agissent sur le grain, ces forces viennes des autres grains, des obstacles, gravité, archimède...
    virtual void ajouteForce();

    virtual Vecteur3D ajouteForce(std:: unique_ptr<Grain> const& grain2) =0;
    ...
    // copie polymorphique
    virtual std::unique_ptr<Grain> copie() const = 0;
