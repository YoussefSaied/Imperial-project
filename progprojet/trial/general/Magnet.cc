...
Magnet :: Magnet(Position const& position, SupportADessin* support, Vitesse const& vitesse, double masse_volumique, double rayon, Vecteur3D const& force,  Medium medium, double height, Vecteur3D moment):
    Dessinable(position,support), velocity(vitesse), masse_volumique(masse_volumique), radius(rayon), force(force), medium(&medium), height(height), moment(moment) {}
