...
public:
    ...
    GrainLJ(Position const& position, Vitesse const& vitesse,
            double masse_volumique, double rayon, Vecteur3D const& force = Vecteur3D(), Medium medium = water, SupportADessin* support = &Texte1);
    virtual ~GrainLJ() {}
    ...
    virtual Vecteur3D ajouteForce(std:: unique_ptr<Grain> const& grain2) override;
	...
	std:: unique_ptr<GrainLJ> cloneMe() const;

    virtual std::unique_ptr<Grain> copie() const override;
...
std :: ostream& operator<<(std :: ostream& c, GrainLJ g);
